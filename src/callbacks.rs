...
type DeferredFuncThreaded = Box<dyn Fn() -> Result<(), String> + Send + Sync>;

type CallbackChannel = (flume::Sender<DeferredFunc>, flume::Receiver<DeferredFunc>);

type CallbackChannelThreaded = (flume::Sender<DeferredFuncThreaded>, flume::Receiver<DeferredFuncThreaded>);

static mut CALLBACK_CHANNELS: Option<[CallbackChannel; 2]> = None;

static mut CALLBACK_CHANNELS_THREADED: Option<[CallbackChannelThreaded; 2]> = None;

pub(crate) const ADJACENCIES: usize = 0;

pub(crate) const DEFERRED_AIRS: usize = 0;

#[init(partial)]
fn _start_aux_callbacks() -> Result<(), String> {
	unsafe {
		CALLBACK_CHANNELS = Some([flume::unbounded(), flume::unbounded()]);
		CALLBACK_CHANNELS_THREADED = Some([flume::unbounded(), flume::unbounded()]);
	}
	Ok(())
}
...
fn _clean_aux_callbacks() {
	unsafe {
		CALLBACK_CHANNELS = None;
		CALLBACK_CHANNELS_THREADED = None;
	}
}
...
fn with_aux_threaded_callback_receiver<T>(
	f: impl Fn(&flume::Receiver<DeferredFuncThreaded>) -> T,
	item: usize,
) -> T {
	f(unsafe { &CALLBACK_CHANNELS_THREADED.as_ref().unwrap()[item].1 })
}
...
pub fn aux_callbacks_sender_threaded(item: usize) -> flume::Sender<DeferredFuncThreaded> {
	unsafe { CALLBACK_CHANNELS_THREADED.as_ref().unwrap()[item].0.clone() }
}
...
pub fn process_aux_callbacks_threaded(item: usize) {
	...
	with_aux_threaded_callback_receiver(
		|receiver| {
			for callback in receiver.try_iter() {
				if let Err(e) = callback() {
					let _ = sender.try_send(Box::new(move || {
						let stack_trace = Proc::find("/proc/auxtools_stack_trace").unwrap();
						let _ =
							stack_trace.call(&[&Value::from_string(e.as_str()).unwrap()]);
						Ok(Value::null())
					}));
				}
			}
